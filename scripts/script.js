...
const renderArticleElement = () => {
    MockupData.forEach((data, index) => {
        data.id = index+1;
        const article = createArticleElement(data);
        document.getElementsByClassName('article-container')[0].appendChild(article);
    });
}
renderArticleElement();

const filterContentByString = () => {
    const inputValue = document.querySelector('input[name="q"]').value.toLowerCase();
    const articles = document.getElementsByClassName('article-container')[0].getElementsByClassName('article-card');
...
const showArticleDetails = (event) => {
    ...
    if (!event.currentTarget){ 
        ...
        basicActiveDetail.lastClickedId = event.currentTarget.id;
